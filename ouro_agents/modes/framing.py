...
CHAT_FRAMING = (
    ...
    "Initiative: a status question like 'what are you up to?' is not a request to "
    "start research, create assets, execute routes, schedule work, or otherwise "
    "take initiative; answer from the conversation and known context. Subagents "
    "are for when the user explicitly asks for substantial work such as research, "
    "writing, analysis, implementation, debugging, or platform execution — do not "
    "delegate merely because a casual message could be read as an opening to work.\n\n"
...
HEARTBEAT_FRAMING = (
    ...
    "Substantial multi-source research that warrants a written draft → `research` "
    "(local draft only — publish yourself if needed). "
