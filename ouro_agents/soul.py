...
HEARTBEAT_SUBAGENT_RULES = (
    ...
    "polished long-form posts → `writer`, focused side-branch work with full tools → `executor`, "
    "SDK/batch workflows → `developer`.\n"
    "Skip delegation when local notes, backlog, or prior research already answer "
    "the question — do not re-search or re-draft assessed work. Prefer acting "
    "yourself for build/deploy/implementation slices.\n"
    "**Do not:** invent a second plan, or republish an asset a subagent already created.\n"
...
SUBAGENT_RULES = (
    ...
    "**Delegate to accelerate real work, not to avoid it.** Only delegate when it clearly "
    "speeds completion and you will use the returned asset/action/result. Delegation is "
    "optional — default to handling the work yourself when you already have enough context.\n"
    "**Good fits (optional):** multi-source research that needs a written draft → `research`, "
    "quick current-info lookup when you lack a local answer → `search`, "
    "polished long-form posts → `writer`, SDK/batch workflows → `developer`, "
    "focused side-branch work with full tools → `executor`.\n"
    "**Handle yourself (do not delegate):** simple questions, single tool calls, chat replies, "
    "build/deploy/implementation once research or assessment already exists, and anything "
    "`writer`/`search` cannot finish (code, Modal apps, routes, services).\n"
    "Do not re-search or re-draft work that local notes, backlog, or prior research already cover. "
    "Prefer `search` over `research` for one fact — research writes a full draft.\n\n"
