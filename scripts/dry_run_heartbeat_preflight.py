"""Dry-run the strong heartbeat preflight for an agent without executing the tick.

Rebuilds the exact task context a real heartbeat would hand to the preflight
(inbox playbook or general playbook + work direction), then runs only the
preflight subagent (memory_recall only — no side effects) and prints the
parsed strategist brief plus the execution brief the mid-tier executor
would receive.

Usage:
    python scripts/dry_run_heartbeat_preflight.py --config hermes.json [-n 2]
"""
...
from ouro_agents.subagents.preflight import format_heartbeat_execution_brief
...
def main() -> None:
    ...
    print(f"=== Heartbeat context source: {source}; team={ctx.team_id or 'none'} ===")
    if not ctx.playbook:
        print("No playbook or inbox work — heartbeat would do nothing.")
        sys.exit(0)
    print(f"Task context: {len(ctx.playbook)} chars")
    if args.show_task:
        print("\n----- TASK CONTEXT -----\n" + ctx.playbook + "\n----- END TASK -----\n")
    ...
    for i in range(args.runs):
        print(f"\n########## PREFLIGHT RUN {i + 1}/{args.runs} ##########")
        result = agent._run_preflight(
            ctx.playbook,
