---
last_updated: 2026-07-27T13:45:00+00:00
---
...
- **Email budget.** Before picking priorities 1-3, count today's outbound emails from **Resend** (`list_emails`, paginated, from `hermes@agents.ouro.foundation`, bounded to today in `America/Chicago`) — not from CRM `date_sent`. The period log is a sanity check only. Every outbound email counts toward the daily cap of 4. Once the count is at 4, the only email still allowed is a live-thread reply (priority 1) — never leave a warm reply waiting a day — and it still counts. Cold sends and follow-ups never exceed the cap: skip to priority 4 or 5.
...
1. **Advance a live conversation.** If someone replied to your outreach, respond — following the thread-reading procedure in `skills/outreach.md` ("Read the full thread before every reply"), no exceptions: re-read **both** Resend received and sent mail, and do not re-answer questions you already settled. **If Matt or Will has replied on the thread himself, the conversation is theirs: do not chime in unless they explicitly ask you to.** Otherwise move toward the next step: an introduction, a relevant quest, a concrete way to share their work or sponsor work. A warm reply left sitting is the most expensive thing you can waste.
